...
describe('Comment (e2e)', () => {
  it('can create', async () => {
    await request("localhost:3001")
      .post('/comment').auth("123", { type: "bearer" }).send
      ({ "description": "test", "authorId": 1, "postId": 1 })
      .expect(201)
  });

  it('can get all', async () => {
    const res = await request("localhost:3001").get('/comment').auth("123", { type: "bearer" });
  ...
  it('can get by id', async () => {
    const res = await request("localhost:3001")
      .get("/comment/1").auth("123", { type: "bearer" });
  ...
  it('can filter by user id', async () => {
    let res = await request("localhost:3001/comment/user/1")
